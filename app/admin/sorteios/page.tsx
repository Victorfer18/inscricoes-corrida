...
export default function SorteiosPage() {
  ...
  const [inscricoesDisponiveis, setInscricoesDisponiveis] = useState<
    InscricaoConfirmada[]
  >([]);
  ...
  const buscarInscricoes = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/admin/sorteios/inscricoes-confirmadas?lote_id=${loteId}`,
        {
          headers: {
  ...
  const realizarSorteio = () => {
    ...
    const intervalo = setInterval(() => {
      const randomIndex = Math.floor(
        Math.random() * inscricoesDisponiveis.length,
      );
      setUltimoSorteado({
  ...
  const exportarResumo = () => {
    ...
    doc.setFontSize(12);
    doc.setFont("helvetica", "bold");
    doc.text(tituloSorteio || "Sorteio de Inscritos", 105, 30, {
      align: "center",
    });
  ...
  return (
    <AdminPageLayout>
                    ...
                    <div className="mb-6 p-6 bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-lg border-2 border-purple-200 dark:border-purple-700">
                      ...
                      <div className={sorteando ? "animate-pulse" : ""}>
                        ...
                        <div className="space-y-1 text-sm text-gray-700 dark:text-gray-300">
                          <p>
                            <strong>CPF:</strong>{" "}
                            {formatarCPF(ultimoSorteado.cpf)}
                          </p>
                          <p>
                            <strong>Email:</strong> {ultimoSorteado.email}
                          </p>
                          <p>
                            <strong>Idade:</strong> {ultimoSorteado.idade} anos
                            • {ultimoSorteado.sexo}
                          </p>
                        </div>
                      </div>
        ...
        <Card>
          <CardBody className="text-center py-12">
            ...
            <p className="text-lg text-gray-600 dark:text-gray-400">
              Selecione um lote e clique em &ldquo;Buscar Inscritos&rdquo; para começar
            </p>
          </CardBody>
        </Card>
      )}
      ...
      <Modal isOpen={isOpenFinalizar} size="3xl" onClose={onCloseFinalizar}>
        <ModalContent>
          {(onCloseFinalizar) => (
            <>
              <ModalHeader>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-4">
                    <Input
                      ...
                      value={tituloSorteio}
                      onValueChange={setTituloSorteio}
                      isRequired
                      variant="bordered"
                    />
                    ...
                    <Textarea
                      ...
                      onValueChange={setDescricaoSorteio}
                      minRows={3}
                      variant="bordered"
                    />
                  </div>
                  ...
                  <div className="max-h-[300px] overflow-y-auto">
                    <Table
                      aria-label="Tabela de sorteados"
                      className="text-sm"
                      removeWrapper
                    >
                      <TableHeader>
                      ...
                      <TableBody>
                        {sorteados.map((sorteado) => (
                          <TableRow key={sorteado.id}>
                            <TableCell>{sorteado.rodada}</TableCell>
                            <TableCell>{sorteado.nome_completo}</TableCell>
                            <TableCell>{formatarCPF(sorteado.cpf)}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
                    ...
                    <TableBody>
                      {sorteados.map((sorteado) => (
                        <TableRow key={sorteado.id}>
                          <TableCell>{sorteado.rodada}</TableCell>
                          <TableCell>{sorteado.nome_completo}</TableCell>
                          <TableCell className="text-xs">{sorteado.email}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
