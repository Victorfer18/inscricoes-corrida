...
import {
  InscricoesTable,
  type InscricaoTableData,
  type PaginationData,
} from "@/components/admin/inscricoes-table";
...
export default function AdminDashboardPage() {
  ...
  const [selectedInscricao, setSelectedInscricao] =
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      ...
      if (editingStatus === "confirmado") {
        const hasExistingComprovante =
          selectedInscricao.comprovante_url &&
          selectedInscricao.comprovante_url.trim() !== "";
      ...
      if (selectedFile) {
        ...
        const uploadResponse = await fetch(
          `/api/admin/inscricoes/${selectedInscricao.id}/comprovante`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
      ...
      const response = await fetch(
        `/api/admin/inscricoes/${selectedInscricao.id}`,
        {
          method: "PUT",
          headers: {
                      ...
                      <div className="space-y-4">
                        <div>
                          <p className="font-semibold">
                            {selectedInscricao.nome_completo}
                          </p>
                          <p className="text-sm text-gray-500">
                            {selectedInscricao.email}
                          </p>
                          <p className="text-sm text-gray-500">
                            CPF: {formatarCPF(selectedInscricao.cpf)}
                          </p>
                        </div>
                        ...
                        <div className="space-y-2">
                          <Select
                            ...
                            onSelectionChange={(keys) =>
                              setEditingStatus(Array.from(keys)[0] as string)
                            }
                          >
                            <SelectItem key="pendente">Pendente</SelectItem>
                            <SelectItem key="confirmado">Confirmado</SelectItem>
                          ...
                          {editingStatus === "confirmado" &&
                            ...
                            !selectedFile && (
                              <div className="flex items-center gap-2 text-amber-600 text-xs bg-amber-50 p-2 rounded-lg">
                                ...
                                <span>
                                  ⚠️ Para confirmar é necessário anexar um
                                  comprovante
                                </span>
                              </div>
                            )}
                        ...
                        <div className="space-y-2">
                          ...
                          {selectedInscricao.comprovante_url ? (
                            <Link
                              ...
                              target="_blank"
                            >
                              {selectedInscricao.comprovante_url.includes(
                                ".pdf",
                              ) ? (
