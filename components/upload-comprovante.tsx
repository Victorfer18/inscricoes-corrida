...
export function UploadComprovante({
  ...
  const handleFileSelect = (file: File) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      alert(
        "Por favor, selecione apenas arquivos de imagem (JPG, PNG, GIF, WebP, BMP, TIFF) ou PDF",
      );
      ...
      <CardBody className="p-4 sm:p-6">
        <div className="text-center mb-4">
          <div className="bg-blue-100 dark:bg-blue-900 p-3 sm:p-4 rounded-full inline-block mb-4">
            <UploadIcon
              className="text-blue-600 dark:text-blue-400 sm:w-8 sm:h-8"
              size={24}
            />
          </div>
          ...
          >
            <div className="space-y-3 sm:space-y-4">
              ...
              <div className="text-xs text-blue-500 dark:text-blue-400 px-2">
                Formatos aceitos:
                <br className="sm:hidden" /> JPG, PNG, GIF, PDF
                <br className="sm:hidden" />• Imagens: 5MB • PDF: 10MB
              </div>
            </div>
          </div>
        ) : (
